...
/**
 * Switches the default database connection when a tenant context becomes active.
 *
 * When the tenant isolation mode is set to `IsolationMode::SEPARATE_DATABASE`, this
 * task resolves the target connection name from the tenant model's `databaseConfig()`
 * (if it implements `DatabaseAwareTenantInterface`) or falls back to the
 * `tenancy.database.connection` config value. It then sets `database.default` to the
 * resolved connection and purges the connection from the manager to ensure a fresh
 * connection is used.
 *
 * A stack-based approach is used to support nested tenant contexts: each
 ...
 */
final class SwitchTenantDatabaseTask implements TaskInterface
{
    /**
     ...
     * by `forgetCurrent` even when tenant contexts are nested.
     *
    ...
    /**
     * Switch the default database connection to the tenant's dedicated connection.
     *
    ...
    public function makeCurrent(TenantContext $tenantContext): void
    {
        ...
        if ($mode === IsolationMode::SHARED_DATABASE) {
            $this->switchedConnectionStack[] = null;

            return;
        }

        $connection = $this->resolveConnectionName($tenantContext);
    ...
    /**
     * Resolve the configured tenant database isolation mode.
     *
     * Reads `tenancy.isolation` from config and returns the corresponding
    ...
    {
        $mode = config('tenancy.isolation', IsolationMode::SHARED_DATABASE->value);
    ...
    /**
     * Resolve the database connection name for the given tenant context.
     *
     * First attempts to read the connection from the tenant model's own
     * `databaseConfig()` array (available when the model implements
     * `DatabaseAwareTenantInterface`). Falls back to the static
     * `tenancy.database.connection` config value when the model does not implement
     * the interface or does not specify a connection.
     *
     * @param  TenantContext $tenantContext The tenant context to resolve a connection for.
     * @return null|string   The resolved connection name, or `null` if none can be determined.
     */
    private function resolveConnectionName(TenantContext $tenantContext): ?string
    {
        $fallback = config('tenancy.database.connection');
