...
/**
 * Optionally resolve and activate a tenant context for the duration of the request.
 *
 * Attempts to identify a tenant from the incoming request using the configured
 * resolver. Unlike `RequireTenant`, this middleware does not abort when no
 * tenant is found — the request proceeds regardless.
 *
 ...
 * the pipeline throws.
 *
 * Use this middleware on routes that may optionally belong to a tenant but
 * should also be accessible without one.
 *
 * @author Brian Faust <brian@cline.sh>
 * @psalm-immutable
 */
final readonly class OptionalTenant
{
    /**
    ...
    /**
     * Attempt to resolve a tenant context and pass the request to the next handler.
     *
    ...
    {
        $this->tenancy->resolveTenant($request);
